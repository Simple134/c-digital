...
export default function AgendarPage() {
  ...
  const goNext = (from: number) => {
    if (from === 1) {
      if (!name.trim() || !role.trim() || !email.trim() || !phone.trim()) {
        alert("Por favor completa todos los campos de contacto."); return;
      }
    }
    if (from === 2) {
      if (!business.trim() || !sector || !stage) {
        alert("Por favor completa el nombre, sector y etapa del negocio."); return;
      }
    }
    if (from === 3) {
      if (!challenge) { alert("Por favor selecciona tu principal desafío."); return; }
    }
    if (from === 4) {
      if (!selectedDate || !selectedTime) {
        alert("Por favor selecciona una fecha y un horario."); return;
      }
    }
    setStep(from + 1);
  ...
  const getCalSelectedDisplay = () => {
    if (selectedDate && selectedTime) {
      return <><strong>{DAYS_ES[selectedDate.getDay()]}, {selectedDate.getDate()} de {MONTHS_ES[selectedDate.getMonth()]} — {selectedTime}</strong></>;
    }
    if (selectedDate) {
      return <>{DAYS_ES[selectedDate.getDay()]}, {selectedDate.getDate()} de {MONTHS_ES[selectedDate.getMonth()]} — <span style={{ opacity: 0.5 }}>selecciona un horario</span></>;
    }
    return null;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!budget) { alert("Por favor selecciona un rango de presupuesto."); return; }
    ...
    const submitData = {
      ...
      "Dia de reunion": selectedDate && selectedTime
  ...
  return (
    <>
      ...
      <div className="book-layout" style={{ background: "#0d0d0d", color: "#fff" }}>

        {/* Panel izquierdo */}
        <aside className="book-info">
          <span className="book-tag">Consultoría Gratuita</span>
          <h1 className="book-title">Agendar<br /><span style={{ fontStyle: "italic" }}>consulta.</span></h1>
          <p className="book-desc">Esta sesión de 30 minutos está diseñada para evaluar tu negocio y diseñar un plan personalizado de digitalización que genere resultados reales.</p>

          <p className="book-discover-label">Durante la consultoría descubriremos:</p>
          ...
          <div className="book-duration">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ opacity: 0.4 }}><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
            30 minutos · Sin costo · Sin compromiso
          </div>

          <p className="book-note">El brief toma 3–5 minutos. Así llegamos preparados con ideas específicas para ti.</p>
        </aside>
        ...
        <main className="book-form-panel">
          ...
          <div className="book-progress">
            {[1,2,3,4,5].map(i => (
              <span key={i} className={step > i || confirmed ? "done" : step === i ? "done" : ""} />
            ))}
          </div>
          ...
          {!confirmed ? (
            <form onSubmit={handleSubmit}>
              ...
              {step === 1 && (
                <div>
                  <p className="step-tag">Paso 1 de 5 — Información de Contacto</p>
                  <h2 className="step-heading">¿Con quién<br />tenemos el gusto?</h2>
                  <div className="field-block">
                    <div className="field-row">
                      <div className="field-group">
                        <input className="field-input" type="text" placeholder="Tu nombre completo" value={name} onChange={e => setName(e.target.value)} required />
                      </div>
                      <div className="field-group">
                        <input className="field-input" type="text" placeholder="Dueño, Gerente, Marketing..." value={role} onChange={e => setRole(e.target.value)} required />
                      </div>
                    </div>
                    <div className="field-group">
                    ...
                    <div className="field-group">
                      <div className="phone-row">
                        ...
                        <input className="field-input" type="tel" placeholder="(809) 000-0000" value={phone} onChange={e => setPhone(e.target.value)} required />
                      </div>
                    </div>
                  </div>
                  <div className="step-nav">
                    <button type="button" className="nav-next" onClick={() => goNext(1)}>
              ...
              {step === 2 && (
                <div>
                  ...
                  <h2 className="step-heading">Cuéntanos sobre<br />tu negocio</h2>
                  <div className="field-block">
                    <div className="field-group">
                      <input className="field-input" type="text" placeholder="Nombre de tu empresa o proyecto" value={business} onChange={e => setBusiness(e.target.value)} required />
                    </div>
                    <div className="field-group">
                      <span className="field-label-top">¿A qué sector pertenece?*</span>
                      <div className="opt-grid">
                        {["Restaurante / Gastronomía","Retail / Comercio","Servicios Profesionales","Salud / Wellness","Tecnología / Software","Construcción / Inmobiliaria"].map(s => (
                          <label key={s} className={`opt-card${sector === s ? " selected" : ""}`}>
                            <input type="radio" name="sector" value={s} checked={sector === s} onChange={() => setSector(s)} />
                            <span>{s}</span>
                          </label>
                        ))}
                        <label className={`opt-card opt-card--full${sector === "Otro" ? " selected" : ""}`}>
                          <input type="radio" name="sector" value="Otro" checked={sector === "Otro"} onChange={() => setSector("Otro")} />
                          <span>Otro</span>
                        </label>
                      </div>
                    </div>
                    <div className="field-group">
                      <span className="field-label-top">¿En qué etapa está tu negocio?*</span>
                      <div className="opt-grid">
                        {[
                          ["Idea / Emprendimiento","Idea / Emprendimiento"],
                          ["Inicio (menos de 1 año)","Inicio — menos de 1 año"],
                          ["En crecimiento (1–3 años)","En crecimiento — 1 a 3 años"],
                          ["Establecido (más de 3 años)","Establecido — más de 3 años"],
                        ].map(([val, label]) => (
                          <label key={val} className={`opt-card${stage === val ? " selected" : ""}`}>
                            <input type="radio" name="stage" value={val} checked={stage === val} onChange={() => setStage(val)} />
                            <span>{label}</span>
                          </label>
                        ))}
                      </div>
                    ...
                    <div className="field-group">
                      <span className="field-label-top">¿Qué tienes actualmente en digital? <span style={{ opacity: .5, fontWeight: 500, textTransform: "none", letterSpacing: 0 }}>(Puedes marcar varios)</span></span>
                      <div className="opt-grid">
                        {["Logo / marca diseñada","Página web","Redes sociales activas","Publicidad digital","Tienda online"].map(d => (
                          <label key={d} className={`opt-card${digital.includes(d) ? " selected" : ""}`}>
                            <input type="checkbox" checked={digital.includes(d)} onChange={() => setDigital(prev => toggleArray(prev, d))} />
                            <span>{d}</span>
                          </label>
                        ))}
                        <label className={`opt-card opt-card--full${digital.includes("Nada aún — Empiezo desde cero") ? " selected" : ""}`}>
                          <input type="checkbox" checked={digital.includes("Nada aún — Empiezo desde cero")} onChange={() => setDigital(prev => toggleArray(prev, "Nada aún — Empiezo desde cero"))} />
                  ...
                  <div className="step-nav">
                    <button type="button" className="nav-back" onClick={() => goPrev(2)}>← Atrás</button>
                    <button type="button" className="nav-next" onClick={() => goNext(2)}>Continuar <ArrowRight /></button>
                  </div>
                </div>
              )}
              ...
              {step === 3 && (
                <div>
                  ...
                  <h2 className="step-heading">¿Qué quieres<br />resolver?</h2>
                  <div className="field-block">
                    <div className="field-group">
                      <span className="field-label-top">¿Cuál es tu principal desafío digital?*</span>
                      <div className="opt-grid">
                        {[
                          ...
                          <label key={c} className={`opt-card opt-card--full${challenge === c ? " selected" : ""}`}>
                            <input type="radio" name="challenge" value={c} checked={challenge === c} onChange={() => setChallenge(c)} />
                            <span>{c}</span>
                          </label>
                        ))}
                      </div>
                    ...
                    <div className="field-group">
                      <span className="field-label-top">¿Qué servicios te interesan? <span style={{ opacity: .5, fontWeight: 500, textTransform: "none", letterSpacing: 0 }}>(Puedes marcar varios)</span></span>
                      <div className="opt-grid">
                        {["Diseño de Marca","Desarrollo Web","Tienda Online","Marketing Digital","Posicionamiento SEO","Community Manager","Sistema Empresarial"].map(s => (
                          <label key={s} className={`opt-card${services.includes(s) ? " selected" : ""}`}>
                            <input type="checkbox" checked={services.includes(s)} onChange={() => setServices(prev => toggleArray(prev, s))} />
                            <span>{s}</span>
                          </label>
                        ))}
                        <label className={`opt-card opt-card--full${services.includes("No estoy seguro — Necesito asesoría") ? " selected" : ""}`}>
                          <input type="checkbox" checked={services.includes("No estoy seguro — Necesito asesoría")} onChange={() => setServices(prev => toggleArray(prev, "No estoy seguro — Necesito asesoría"))} />
                  ...
                  <div className="step-nav">
                    <button type="button" className="nav-back" onClick={() => goPrev(3)}>← Atrás</button>
                    <button type="button" className="nav-next" onClick={() => goNext(3)}>Continuar <ArrowRight /></button>
                  </div>
                </div>
              )}
              ...
              {step === 4 && (
                <div>
                  ...
                  <h2 className="step-heading">¿Cuándo<br />nos reunimos?</h2>
                  <div className="field-block">
                    <div className="field-group">
                      <div className="cal-wrap">
                        <div className="cal-nav">
                          <button type="button" className="cal-arrow" onClick={calPrev}>‹</button>
                          <span className="cal-month-label">{MONTHS_ES[calMonth]} {calYear}</span>
                          <button type="button" className="cal-arrow" onClick={calNext}>›</button>
                        </div>
                        ...
                        <div className="cal-grid">
                          ...
                          {Array.from({ length: daysInMonth }, (_, i) => {
                            ...
                            const isSelected = selectedDate?.toDateString() === thisDay.toDateString();
                            ...
                            return (
                              <button
                                key={d} type="button" className={cls}
                                disabled={disabled}
                                onClick={() => { if (!disabled) { setSelectedDate(thisDay); setSelectedTime(null); } }}
                              >{d}</button>
                            );
                          })}
                        </div>
                      ...
                      <div className="time-section">
                        ...
                        <div className="cal-selected-display">{getCalSelectedDisplay()}</div>
                      </div>
                    </div>
                  </div>
                  <div className="step-nav">
                    <button type="button" className="nav-back" onClick={() => goPrev(4)}>← Atrás</button>
                    <button type="button" className="nav-next" onClick={() => goNext(4)}>Continuar <ArrowRight /></button>
                  </div>
                </div>
              )}
              ...
              {step === 5 && (
                <div>
                  ...
                  <h2 className="step-heading">¿Cuál es tu<br />presupuesto aproximado?</h2>
                  <div className="field-block">
                    <div className="field-group">
                      <div className="opt-grid">
                        {[
                          ["Menos de $1,000 USD","Presupuesto ajustado","Menos de $1,000 USD"],
                          ["$1,000 - $2,000 USD","Presupuesto moderado","$1,000 – $2,000 USD"],
                          ["$2,000 - $4,000 USD","Presupuesto flexible","$2,000 – $4,000 USD"],
                          ["Más de $4,000 USD","Presupuesto amplio","Más de $4,000 USD"],
                        ].map(([val, label, range]) => (
                          <label key={val} className={`opt-card opt-card--full${budget === val ? " selected" : ""}`}>
                            <input type="radio" name="budget" value={val} checked={budget === val} onChange={() => setBudget(val)} />
                            <span><strong>{label}</strong> &nbsp;—&nbsp; {range}</span>
                          </label>
                        ))}
                        <label className={`opt-card opt-card--full${budget === "Aún explorando opciones" ? " selected" : ""}`}>
                          <input type="radio" name="budget" value="Aún explorando opciones" checked={budget === "Aún explorando opciones"} onChange={() => setBudget("Aún explorando opciones")} />
                    ...
                    <p className="form-legal">Al enviar este formulario, aceptas que C Digital utilice tu información para contactarte. Tus datos están seguros y nunca serán compartidos con terceros.</p>
                  </div>
                  <div className="step-nav">
                    <button type="button" className="nav-back" onClick={() => goPrev(5)}>← Atrás</button>
                    <button type="submit" className="nav-submit" disabled={isSubmitting}>
            ...
            <div className="book-confirm">
              ...
              <p>Recibimos tu brief. Nos ponemos en contacto en menos de 24 horas para confirmar la sesión y llegar preparados con ideas específicas para ti.</p>
              <Link href="/" className="confirm-back">← Volver al inicio</Link>
            </div>
          )}

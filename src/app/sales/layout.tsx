  ...
  return (
    <Container className="!w-full bg-transparent relative z-[100]">
      <div className="flex justify-between items-center text-white !w-full pt-4 relative z-[100]">
        ...
        <div className="space-x-8 lg:block hidden relative z-50">
          {[
            ...
            <Link
              key={label}
              href={href}
              className={pathname === href ? "bg-gradient-to-r from-[#00C5FF] to-[#00FF7C] text-transparent bg-clip-text font-bold" : "hover:text-[#9F9F9F] font-normal"}
            >
              {label}
            </Link>
          ))}
        ...
        <div className={`fixed top-0 right-0 h-screen w-64 bg-black/95 backdrop-blur-lg transform transition-transform duration-300 ease-in-out z-40 lg:hidden ${isMenuOpen ? "translate-x-0" : "translate-x-full"}`}>
    ...
    <footer className="bg-black py-16 relative">
      <Container>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-12">
          <div className="flex flex-col space-y-6">
            ...
            <p className="text-gray-400 text-sm leading-relaxed max-w-xs">
              Somos un estudio de diseño especializado en soluciones digitales para empresas. Si buscas crecer necesitas C Digital.
            </p>
          ...
          <div className="lg:ml-12">
            ...
            <ul className="space-y-4">
              {[{ name: "Home", href: "/" }, { name: "Servicios", href: "/servicios" }, { name: "Contacto", href: "/contacto" }].map((item) => (
                <li key={item.name}>
                  <Link href={item.href} className="text-gray-400 hover:text-white transition-colors text-sm">{item.name}</Link>
                </li>
              ))}
            </ul>
          </div>
            ...
            <ul className="space-y-4">
              <li>
                <a href="tel:+17867557025" className="flex items-center gap-3 text-gray-400 hover:text-white transition-colors text-sm">
                  <Phone className="w-4 h-4" /><span>+1 786-755-7025</span>
                </a>
              </li>
              <li>
                <a href="mailto:hola@estudiocdigital.com" className="flex items-center gap-3 text-gray-400 hover:text-white transition-colors text-sm">
                  <Mail className="w-4 h-4" /><span>hola@estudiocdigital.com</span>
                </a>
              </li>
        ...
        <div className="pt-8 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-gray-500 text-sm">© {new Date().getFullYear()} C Digital. Todos los derechos reservados.</p>
          <div className="flex gap-6">
            <Link href="/privacidad" className="text-gray-500 hover:text-white text-sm transition-colors">Política de Privacidad</Link>
            <Link href="/terminos" className="text-gray-500 hover:text-white text-sm transition-colors">Términos de Uso</Link>
          </div>
        </div>
      </Container>
    </footer>
